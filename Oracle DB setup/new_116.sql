...
BEFORE INSERT ON Etudiants_Maroc
FOR EACH ROW
DECLARE
	n NUMBER;
	etudiant_duplique EXCEPTION;
BEGIN
  SELECT COUNT(*) INTO n FROM Etudiants_Maroc WHERE Nom = :NEW.Nom AND Prenom = :NEW.Prenom;
  IF n = 0 THEN
...
INSERT INTO Etudiants_Maroc VALUES(sq_Id_Etudiant.NEXTVAL, 'EN-NAHEL', 'Aissam', 'Marocain', 0);

INSERT INTO Etudiants_Maroc VALUES(sq_Id_Etudiant.NEXTVAL, 'EN-NAHEL', 'Hicham', 'Marocain', 0);
...
BEFORE INSERT ON Universites_Maroc
FOR EACH ROW
DECLARE
	n NUMBER;
...
INSERT INTO Universites_Maroc VALUES(sq_Id_Univetsite.NEXTVAL, 'Université Ibn Zohr', 'Agadir');
INSERT INTO Universites_Maroc VALUES(sq_Id_Univetsite.NEXTVAL, 'Université Mohamed 2', 'Agadir');
...
BEFORE INSERT ON Etud_Univ_Maroc
FOR EACH ROW
DECLARE
	n NUMBER;
	etudiant_deja EXCEPTION;
BEGIN
  SELECT COUNT(*) INTO n FROM Etud_Univ_Maroc WHERE ID_etudiant = :NEW.ID_etudiant;
  IF n = 0 THEN
...
INSERT INTO Etud_Univ_Maroc VALUES(, );
...
BEFORE INSERT ON Bourses_Maroc
FOR EACH ROW
DECLARE
	n NUMBER;
	etudiant_deja EXCEPTION;
...
CLEAR SCREEN
CREATE OR REPLACE PROCEDURE INSERT_universite(p_Nom Universites_Maroc.Nom%TYPE, 
                        p_Localisation Universites_Maroc.Localisation%TYPE ) AS					
  n NUMBER := 0;
  universite_duplique EXCEPTION;
	
  BEGIN
    SELECT COUNT(*) INTO n FROM Universites_Maroc WHERE Nom = p_Nom;
    IF n = 0 THEN
	  -- verifier si l'universite n'existe pas
      -- Inserer la ligne dans la table Universites_Maroc
      INSERT INTO Universites_Maroc VALUES(sq_Id_Univetsite.NEXTVAL, p_Nom, p_Localisation);
...
CLEAR SCREEN
CREATE OR REPLACE PROCEDURE INSERT_etudiant_universite(p_ID_etudiant Etud_Univ_Maroc.ID_etudiant%TYPE, 
...
CLEAR SCREEN
CREATE OR REPLACE PROCEDURE INSERT_bourse(p_ID_etudiant Bourses_Maroc.ID_etudiant%TYPE, 
...
-- ////// Action : Go to moderateur_bourse_etranger.sql , then complet from Action6 \\\\\\
